...
$user = current_user();
if (!in_array(($user['role'] ?? ''), ['resident', 'admin'], true)) json_response(['ok' => false, 'message' => 'Forbidden'], 403);
$subject = trim($_POST['subject'] ?? '');
$message = trim($_POST['message'] ?? '');
if ($subject === '' || $message === '') json_response(['ok' => false, 'message' => 'Subject and message are required'], 422);
$residentId = null; $houseNumber = null;
...
$stmt->execute([$residentId, $user['name'], $user['role'], $houseNumber, $subject, $message]);
$adminId = admin_user_id();
if ($adminId) add_notification($adminId, 'New trouble ticket', $subject);
add_log('Ticket created API', $subject);
