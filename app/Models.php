...
class BlacklistModel
{
    ...
    public static function add(array $data, int $createdBy): int
    {
        ...
        // Keep every vehicle attached to the request. The first vehicle remains
        // mirrored in visitor_requests for compatibility with existing logs.
        $vehicles = $data['vehicles'] ?? [];
        $vehicleStmt = Database::pdo()->prepare(
            "INSERT INTO visitor_request_vehicles (visitor_request_id, plate_number, vehicle_type, people_count) VALUES (?, ?, ?, ?)"
        );
        foreach ($vehicles as $vehicle) {
            $vPlate = strtoupper(trim((string)($vehicle['plate_number'] ?? '')));
            $vType = strtolower(trim((string)($vehicle['vehicle_type'] ?? 'other')));
            if ($vPlate !== '') {
                $vehicleStmt->execute([$requestId, $vPlate, in_array($vType, ['car','motorcycle','truck','other'], true) ? $vType : 'other']);
            }
        }
...
class VisitorRequestModel
{
    ...
    {
        $stmt = Database::pdo()->prepare("SELECT * FROM visitor_request_vehicles WHERE visitor_request_id = ? ORDER BY id ASC");
